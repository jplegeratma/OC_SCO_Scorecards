...
(SELECT CASE WHEN STATUS = 'Complete' THEN 0 ELSE 1 END AS COMP FROM INF_B_DQ_LOAD_BATCH WHERE PROGRAM_NAME = 'WKF_INF_B_SC_MCO_MBFP_DOS' AND PARENT_BATCH_SEQ = TO_NUMBER('${lgrd_batch_seq}','999999')) b "
...
step_37 ()
{
    ########## Run Application: APP_INF_B_SC_STG_MCO_MBH_REP_FAILS Workflow: WKF_INF_B_SC_STG_MCO_MBH_REP_FAILS
    echo "Info: ($script_name) Executing Workflow  - WKF_INF_B_SC_STG_MCO_MBH_REP_FAILS"
    ...
    while [[ "$success_code" -eq 1 && "$cnt" -le "$cntr" ]]
    do
            infacmd.sh wfs startWorkflow -dn $DOMAIN -sn $INF_SERVER -un $INF_USER -pd $INF_PASS -a APP_INF_B_SC_STG_MCO_MBH_REP_FAILS -wf WKF_INF_B_SC_STG_MCO_MBH_REP_FAILS -pf SC_PARAMETERS.xml
    ...
    if [ "$success_code" -eq 0 ]
       then
            echo "Info: ($script_name) APP_INF_B_SC_STG_MCO_MBH_REP_FAILS completed at: `date` "
            sleep 20
            step_51
       else
            send_email "${email_recepient}" "Error in ($script_name)" "step 37 failed - APP_INF_B_SC_STG_MCO_MBH_REP_FAILS failed "
            error_msg_ex  " step 37, executing APP_INF_B_SC_STG_MCO_MBH_REP_FAILS "
        fi
