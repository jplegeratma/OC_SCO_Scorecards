...
create or replace view MHTEAM.DWDQ.INF_SC_SCO_FAILS(
	RUN_DATE,
	DQ_BATCH_SEQ,
	ID,
	BENCHMARK,
        ...
        FROM ((SELECT f."RUN_DATE",
                 ...
                 FROM (  SELECT RUN_DATE,
                           ...
                           FROM (SELECT RUN_DATE,
                                   ...
                                   FROM MHTEAM.DWDQ.INF_B_SC_STG_SCO_FAILS)
                          WHERE rnk <= 25
...
create or replace view MHTEAM.DWDQ.INF_SC_SCO_PROVIDER_MISS_IDS_BHP(
	S_RUN_DATE,
	S_PROV_ID,
	B_RUN_DATE,
	B_PROV_ID,
...
create or replace view MHTEAM.DWDQ.INF_SC_SCO_PROVIDER_STATS(
	VAL1,
	VAL2,
	VAL7,
	VAL3,
    ...
    SELECT VAL1,
           ...
           R1,
           C1
      FROM MHTEAM.DWDQ.INF_B_SC_STG_SCO_PROVIDER_STATS;

create or replace view MHTEAM.DWDQ.INF_SC_SCO_QUARTER(
	DATE_STR,
	RUN_DATE,
	EMAIL_BODY
) as
    SELECT DISTINCT date_str, run_date, a ||                            --b ||
      ...
      FROM (SELECT date_str,
              ...
              FROM (  SELECT date_str,
                             run_date,
                                'SCO quarterly spreadsheet: <br/>SCO_REPORT_CARD_'
                             || date_str
                             || '.xlsx '
                             || '<br/>SCO provider summary: <br/>Provider_Summary_Evaluation_'
                             || date_str
                             || '.xlsx <br/><br/><br/>'                       a,
                        ...
                        FROM (                   --MHTEAM.DWDQ.INF_SC_STEP3_16_ACO_RUNDATE
                              SELECT RUN_DATE,
                                ...
                                FROM (SELECT DISTINCT
                                       ...
                                       WHERE     p1.PARAM_NAME = 'SO_MIN_DOS'
                                             AND p2.PARAM_NAME = 'SO_RUN_DATE'
                                             AND p3.PARAM_NAME = 'SO_RUN_MNTHS'))
...
create or replace view MHTEAM.DWDQ.INF_SC_SCO_REPORT(
	RUN_DATE,
	MEASURE,
	CLAIMTYPE,
	BENCHMARK,
