...
#if [ ${day_of_week} -eq 5 -a ${day_of_month} -ge 15 -a ${day_of_month} -le 21 ]; then    echo "third Friday "; fi
...
if [ ${day_of_week} -ne 5 -o ${day_of_month} -lt 15 -o ${day_of_month} -gt 21 ]; then exit 1; fi

cd /home/dq_dev/dqs/scheduling/scripts 
