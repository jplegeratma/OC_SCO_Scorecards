#!/bin/sh
# runDQ_SCORECARD_SCO_4Q_back.sh

LOG_FILE=../logs/DQ_SCORECARD_SCO_4Q_BACK_`date +%m%d%y%H%M%S`.sh.log
export LOG_FILE
...
#set the offsets
#4Q run 12 months offset 3 months run
 
#SCO NUMBER OF MONTHS BETWEEN RUN DATE AND MAX DOS
export SO_RUN_DATE_TO_MAX_OFFSET_MNTHS=12
