...
AND ENC.CDE_ENC_MCO IN('CCA','UHC','NAV','SWH','TFT','BHP')
--LIMIT 150
;
...
FROM (
    ...
    (
        SUM(actuals)
        FOR cde_enc_mco IN ( 'BHP', 'CCA', 'NAV', 'SWH', 'TFT', 'UHC' )
    )
)
ORDER BY meas;
...
and cde_enc_mco in ('BHP','UHC','NAV','SWH','TFT','CCA')    
...
from MHTEAM.DWDQ.INF_B_SC_STG_SCO_PROVIDER
where cde_enc_mco in ('BHP','UHC','NAV','SWH','TFT','CCA') 
AND RUN_DATE = TO_DATE('~ASOFDT~','YYYYMMDD')    
